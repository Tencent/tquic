 ...
 */
void quic_config_set_min_ack_delay(quic_config_t *config, uint64_t v);
...
/**
 * @brief Request the peer to update its ACK frequency.
 *
 * This function creates an ACK_FREQUENCY frame and puts it into the send queue.
 * The protocol stack will send this frame in a 1-RTT packet at an appropriate time.
 *
 * @param conn The connection object pointer.
 * @param ack_eliciting_threshold Packet Tolerance, the number of ack-eliciting
 * packets the peer can wait to receive before sending an ACK.
 * @param requested_max_ack_delay The requested maximum ACK delay (in microseconds)
 * that tells the peer how long it can delay sending an ACK after receiving an
 * ack-eliciting packet.
 * @param reordering_threshold Reordering Threshold, how much packet reordering
 * the peer can tolerate before marking a packet as lost.
 *
 * @return 0 on success, or a negative error code on failure.
 */
int quic_conn_update_ack_frequency(quic_conn_t *conn,
